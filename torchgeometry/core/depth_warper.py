...
class DepthWarper(nn.Module):
    ...
    @staticmethod
    def _create_meshgrid(height, width) -> torch.Tensor:
    ...
    def warp_grid(self, depth_src) -> torch.Tensor:
        ...
        pixel_coords = self.grid.to(device).to(dtype).expand(
